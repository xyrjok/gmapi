export default {
  async fetch(request, env) {
    ...
    if (path === '/api/admin/gmails' && request.method === 'POST') {
      ...
      await XYRJ_GMAILAPI.prepare("INSERT INTO gmail_apis (name, script_url, token) VALUES (?, ?, ?)")
    ...
    if (path === '/api/admin/gmails/batch-delete' && request.method === 'POST') {
        ...
        await XYRJ_GMAILAPI.prepare(`DELETE FROM gmail_apis WHERE id IN (${placeholders})`)
            .bind(...ids).run();
        return jsonResp({ success: true });
    }
    ...
    if (path === '/api/admin/gmails/toggle' && request.method === 'POST') {
        ...
        await XYRJ_GMAILAPI.prepare("UPDATE gmail_apis SET is_active = ? WHERE id = ?")
            .bind(status, id).run();
        return jsonResp({ success: true });
    }
